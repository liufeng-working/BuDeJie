...
- (void)setRefreshHeaderEnable:(BOOL)refreshHeaderEnable
{
    if (refreshHeaderEnable) {
        if ([self respondsToSelector:@selector(tableView)]) {
            [self setValue:[LFRefreshHeader headerWithRefreshingTarget:self refreshingAction:@selector(loadData)] forKeyPath:@"tableView.mj_header"];
        }
    }else {
        if ([self respondsToSelector:@selector(tableView)]) {
...
- (void)setRefreshFooterEnable:(BOOL)refreshFooterEnable
{
    if (refreshFooterEnable) {
        if ([self respondsToSelector:@selector(tableView)]) {
            [self setValue:[LFRefreshFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMoreData)] forKeyPath:@"tableView.mj_footer"];
        }
    }else {
        if ([self respondsToSelector:@selector(tableView)]) {
